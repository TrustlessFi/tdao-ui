...
import {
  executeGetGovernor,
  executeGetContract,
  executeGetTrustlessMulticall,
  executeGetProtocolDataAggregator,
} from "./api"
...
export type ProtocolContractsState = {
  [key in ProtocolContract]: sliceState<getContractReturnType>
}
...
const contractsInitialState: ProtocolContractsState = {
  ...
  [ProtocolContract.TcpAllocation]: initialState,
  ...
  extraReducers: (builder) => {
    for (const contractString in ProtocolContract) {
      ...
      if (contract === ProtocolContract.Governor) {
        builder = getContractGenericReducerBuilder<getSingleContractArgs>(
          builder,
          getGovernorContract,
          contract
        )
        ...
        builder = getContractGenericReducerBuilder<getContractArgs>(
          builder,
          getContractThunk(contract),
          contract
        )
      }
    }
