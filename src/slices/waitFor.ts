...
import { getGenesisPositions, getGenesisRounds } from "./genesis"
...
const isProtocolContract = (s: string | number): s is ProtocolContract =>
  s in ProtocolContract
...
const getNodeFetch = (
  ...
  switch (fetchNode) {
    ...
    case ProtocolContract.Governor:
      return {
        [ProtocolContract.Governor]: waitForGovernorContract(
          selector,
          dispatch
        ),
      }
    case ProtocolContract.TrustlessMulticall:
      return {
        [ProtocolContract.TrustlessMulticall]:
          waitForTrustlessMulticallContract(selector, dispatch),
      }
    case ProtocolContract.ProtocolDataAggregator:
      return {
        [ProtocolContract.ProtocolDataAggregator]:
          waitForProtocolDataAggregator(selector, dispatch),
      }

    default:
      if (!isProtocolContract(fetchNode))
        throw new Error("Missing fetchNode " + fetchNode)
      return {
        [fetchNode]: getContractWaitFunction(fetchNode)(selector, dispatch),
      }
  }
}
...
export const waitForProtocolDataAggregator = getWaitFunction(
  (state: RootState) =>
    state.contracts[ProtocolContract.ProtocolDataAggregator],
  getProtocolDataAggregatorContract,
  [FetchNode.ChainID]
)

export const getContractWaitFunction = (protocolContract: ProtocolContract) =>
  getWaitFunction(
...
export const waitForGenesisRounds = getWaitFunction(
  (state: RootState) => state.genesis.rounds,
  getGenesisRounds,
  []
)
